...
void freeBlock(word* block){
  word* free = freelist;
  word** prev = &freelist;
  while(free < block && inHeap(free)){
    prev = (word**)&free[1];
    free = (word*) free[1];
  }
  assert(free != block);
  block[1] = (int) free;
  *prev = block;
  *block = Paint(*block, Blue);
}

int joinFree(word* header)
{
  int size = 0;
  word* block = header;
  do {
    int len = Length(*block)+1;
    size += len;
    block += len;
    assert(len>0);
...
void sweepPhase() {
  printf("sweeping ...\n");
  word* header=heap;
  while(header < afterHeap){
    int color=Color(*header);
    int stepNext = Length(*header) +1;
    switch (color)
    {
      case White:
        assert(Length(*header) >= 2);
        freeBlock(header);
        header += joinFree(header)+1;
        break;
      case Black:
        *header = Paint(*header, White);
        header += stepNext;
        break;
      default:
        header += stepNext;
